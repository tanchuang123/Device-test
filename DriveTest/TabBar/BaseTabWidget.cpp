...

{
    ...
    bar = new BaseTabBar(this);
    setTabBar(bar);
    connect(this, SIGNAL(currentChanged(int)), this, SLOT(switchScreen(int)));//索引
...
void BaseTabWidget::deleteWidget(int index)
{
...
void BaseTabWidget::switchScreen(int index)
{
    int n = 0 ;
    ...
    pixmap = tempPixmap;
    this->widget(preindex)->render(&pixmap);//渲染
...
void BaseTabWidget::cloudAntimation(animation_Direction direction)
{
    ...
    QPropertyAnimation *animation = new QPropertyAnimation(circle, "geometry");//动画效果
...
void BaseTabWidget::resizeEvent(QResizeEvent  *e)
{
    QTabWidget::resizeEvent(e);
    bar->setTabWidth(width());
}

void BaseTabWidget::setTabBarHeight(int value)
{
    bar->setTabHeight(value);


}
void BaseTabWidget::addTabWidget(QString text)
{
    QWidget *widget=new QWidget(this);
    addTab(widget,text);

}
int BaseTabWidget::countNow()
{
    index_= this->count();
    qDebug()<<index_<<"index_";
    return index_;
