...
QRectF Callout::boundingRect() const//边界矩形
{
    QPointF anchor = mapFromParent(m_chart->mapToPosition(m_anchor));
//    qDebug()<<mapFromParent(m_chart->mapToPosition(m_anchor))<<"xxxx";
    QRectF rect;
    rect.setLeft(qMin(m_rect.left(), anchor.x()));
    qDebug()<<qMin(m_rect.left(), anchor.x())<<"";
    rect.setRight(qMax(m_rect.right(), anchor.x()));
    qDebug()<<qMax(m_rect.right(), anchor.x())<<"";
